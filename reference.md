...
评论端点 (`/topics/{id}/comments`) 被知识星球反爬机制拦截，返回 `"不支持非官方工具访问，建议使用官方 Skill 获取内容"`。主题端点不受影响。

脚本处理方式：
1. 评论请求使用 `quiet=True` 静默警告输出
2. 首次评论获取失败后设置 `comments_blocked` 标志，跳过后续所有评论请求
3. Markdown 中记录 `comments_count`（评论数量），即使评论内容未获取
